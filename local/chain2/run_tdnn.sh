...
if [ $stage -le 10 ]; then
  ...
  if [ -d $lang ]; then
    if [ $lang/L.fst -nt $src_langdir/L.fst ]; then
...
if [ $stage -le 23 ]; then
  ...
  utils/mkgraph.sh \
    --self-loop-scale 1.0 ${src_langdir}_2g \
    $tree_dir $tree_dir/graph_2g || exit 1;
fi
